...
export function useMidiDevicesBase(options: UseMidiDevicesBaseOptions) {
  ...
  const loadKeymapForDevice = useCallback(
    (deviceName: string, manufacturer: string): KeymapObject | undefined => {
      const keymaps = (storage.data ?? []) as KeymapObject[];
      return keymaps.find(
  ...
  const saveDevice = useCallback(
    ...
    [executeScript, storage.data, storage.setData]
  );
    ...
    (midiInput: MIDIInput) => {
      setDevices((prev) => {
        ...
        const device = new MIDIDevice(midiInput, MIDI_SERVICE_NAME, saveDevice, {
          executeScript,
          onMessage,
          initialMappings: keymapData?.mappings ?? undefined,
  ...
  return {
    devices,
    requestAccess,
    keymaps: (storage.data ?? []) as KeymapObject[],
